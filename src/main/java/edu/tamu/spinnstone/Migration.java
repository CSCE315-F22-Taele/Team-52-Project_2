...
public class Migration {
  ...
  // public Migration(String username, String password, String databaseUrl) throws
  // SQLException {
  ...
  // String sql = new String(Files.readAllBytes(Path.of(projectRoot,
  // "src/sql/build_tables.sql")));
  ...
  // public ArrayList<Order> generateRandomOrder(ArrayList<MenuItem> menu,
  // ArrayList<Product> products, boolean gameday, Date orderDate) throws
  // SQLException {
  ...
  // OrderItem orderItem = OrderItem.create(this.connection, order.orderId,
  // menuItem.menuItemId);
  ...
  // MenuItem m = MenuItem.create(this.connection, menuItems[i], new
  // BigDecimal(menuItemPrices[i]));
  // menu.add(m);
  // }
  ...
  // // create shipments
  // for (int i = 0; i <= 2; ++i) {
  // Shipment shipment = Shipment.create(connection, new Date(2020,1,(i + 1) * 8),
  // false);
