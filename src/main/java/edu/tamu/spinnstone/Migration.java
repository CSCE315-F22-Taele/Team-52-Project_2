...
import edu.tamu.spinnstone.models.*;
...
public class Migration {
     ...
     * @throws SQLException
     */
    public Order generateRandomOrder(Date orderDate) throws SQLException {
    ...
    public void populate() throws SQLException, IOException {
        ...
        Product.ProductType[] productTypes = new Product.ProductType[]{Product.ProductType.DRINK, Product.ProductType.DRIZZLE, Product.ProductType.SAUCE, Product.ProductType.CHEESE, Product.ProductType.MEAT, Product.ProductType.VEGGIES, Product.ProductType.CRUST, Product.ProductType.OTHER,};

        for (Product.ProductType type : productTypes) {
            db.insert(Table.Names.PRODUCT_TYPE.toString()).columns("product_type_name").values(type.toString()).execute();
        }
        ...
        MenuItem.Categories[] categories = new MenuItem.Categories[]{MenuItem.Categories.PIZZA, MenuItem.Categories.BEVERAGE, MenuItem.Categories.OTHER,};

        for (MenuItem.Categories category : categories) {
            db.insert(Table.Names.MENU_ITEM_CATEGORY.toString()).columns("menu_item_category_name").values(category.toString()).execute();
        }
        ...
        String[][] productDefinitions = {{"Fountain Drink", "5000", "1", Product.ProductType.DRINK.toString()}, {"Bottled Beverage", "5000", "1", Product.ProductType.DRINK.toString()}, {"Gatorade", "1000", "1", Product.ProductType.DRINK.toString()}, {"Cauliflower", "200", "8", Product.ProductType.CRUST.toString()}, {"Standard", "3500", "8", Product.ProductType.CRUST.toString()}, {"Alfredo", "1500", "4", Product.ProductType.SAUCE.toString()}, {"Traditional Red", "3000", "4", Product.ProductType.SAUCE.toString()}, {"Zesty Red", "2500", "4", Product.ProductType.SAUCE.toString()}, {"House Blend", "2000", "6", Product.ProductType.CHEESE.toString()}, {"Parmesan", "2000", "6", Product.ProductType.CHEESE.toString()}, {"BBQ Sauce", "1000", "0.5", Product.ProductType.DRIZZLE.toString()}, {"Olive Oil", "1000", "0.5", Product.ProductType.DRIZZLE.toString()}, {"Oregano", "500", "0.5", Product.ProductType.DRIZZLE.toString()}, {"Ranch", "1000", "0.5", Product.ProductType.DRIZZLE.toString()}, {"Sriracha", "1000", "0.5", Product.ProductType.DRIZZLE.toString()}, {"Diced Ham", "2000", "1", Product.ProductType.MEAT.toString()}, {"Italian Sausage", "2000", "1", Product.ProductType.MEAT.toString()}, {"Meatball", "2000", "1", Product.ProductType.MEAT.toString()}, {"Pepperoni", "2000", "1", Product.ProductType.MEAT.toString()}, {"Salami", "2000", "1", Product.ProductType.MEAT.toString()}, {"Smoked Chicken", "2000", "1", Product.ProductType.MEAT.toString()}, {"Banana Peppers", "2000", "1", Product.ProductType.VEGGIES.toString()}, {"Black Olives", "2000", "1", Product.ProductType.VEGGIES.toString()}, {"Green Peppers", "2000", "1", Product.ProductType.VEGGIES.toString()}, {"Jalapenos", "2000", "1", Product.ProductType.VEGGIES.toString()}, {"Mushrooms", "2000", "1", Product.ProductType.VEGGIES.toString()}, {"Onions", "2000", "1", Product.ProductType.VEGGIES.toString()}, {"Pineapple", "2000", "1", Product.ProductType.VEGGIES.toString()}, {"Roasted Garlic", "2000", "1", Product.ProductType.VEGGIES.toString()}, {"Spinach", "2000", "1", Product.ProductType.VEGGIES.toString()}, {"Tomatoes", "2000", "1", Product.ProductType.VEGGIES.toString()}};

        products = new ArrayList<Product>();

        for (String[] p : productDefinitions) {
            products.add(Product.create(db, p[0], Integer.parseInt(p[1]), Float.parseFloat(p[2]), productTypeMap.get(p[3])));
        }

        // now create the menu items

        String[][] menuItemsDefinitions = {{"1 Topping Pizza", "7.79", MenuItem.Categories.PIZZA.toString()}, {"2-4 Topping Pizza", "8.99", MenuItem.Categories.PIZZA.toString()}, {"Original Cheese Pizza", "6.79", MenuItem.Categories.PIZZA.toString()}, {"Bottled Beverage", "2.39", MenuItem.Categories.BEVERAGE.toString()}, {"Gatorade", "2.39", MenuItem.Categories.BEVERAGE.toString()}, {"Fountain Drink", "1.99", MenuItem.Categories.BEVERAGE.toString()}};

        menuItems = new ArrayList<MenuItem>();

        // Adds menu options to Menu Items
        for (String[] menuItem : menuItemsDefinitions) {
            menuItems.add(MenuItem.create(db, menuItem[0], new BigDecimal(menuItem[1]), categoryMap.get(menuItem[2]), menuItem[2].equals(MenuItem.Categories.PIZZA.toString()) ? true : false));
        }

        String[] pizzaOptions = {"Cauliflower", "Standard", "Alfredo", "Traditional Red", "Zesty Red", "House Blend", "Parmesan", "BBQ Sauce", "Olive Oil", "Oregano", "Ranch", "Sriracha", "Diced Ham", "Italian Sausage", "Meatball", "Pepperoni", "Salami", "Smoked Chicken", "Banana Peppers", "Black Olives", "Green Peppers", "Jalapenos", "Mushrooms", "Onions", "Pineapple", "Roasted Garlic", "Spinach", "Tomatoes"};

        // Adds options (products) to Menu Items
        ResultSet rs = db.query("select menu_item_id from menu_item\n" + "join menu_item_category mic on menu_item.menu_item_category_id = mic.menu_item_category_id\n" + "where mic.menu_item_category_name = 'Pizza'");
        ...
        while (rs.next()) {
            ...
            for (String option : pizzaOptions) {
                ResultSet prs = db.select("product_id").from("product").where("product_name", option).execute();
                ...
                db.insert(Table.Names.MENU_ITEM_PRODUCT.toString()).columns("menu_item_menu_item_id", "product_product_id", "optional").values(mid, pid, true).execute();
            }
        }
